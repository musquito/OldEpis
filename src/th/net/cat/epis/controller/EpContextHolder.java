	...
	public static class EpContext implements Serializable {
		...
		EpContext(JdbcTemplate jdbcTemplate) {
			...
			Map<String, Object> userInfo = jdbcTemplate.queryForMap(
					"SELECT o.officerid AS officeid, o.username AS username, o.officercode AS usercode, o.officerfamilyname AS familyname, o.officergivenname AS givenname, o.sessionid, o.ispositive AS ispositive, p.principalid AS roleid, p.name AS rolename, p.description AS roledesc FROM MASOFFICER o INNER JOIN ARCPRINCIPAL p ON o.principalid = p.principalid WHERE o.username = ?",
					userName);
			this.officerId = Long.valueOf(userInfo.get("officeid") + "");
			Long sessId = userInfo.get("sessionid") == null ? null : Long.valueOf(userInfo.get("sessionid") + "");

			// List<Long> posIds = jdbcTemplate.queryForList("SELECT posid FROM
			// COROFFICERPOS WHERE officerid = ?", new Object[] { this.officerId },
			// Long.class);
			// Map<String, Object> posInfo = posIds.size() < 1 ? new
			// java.util.HashMap<String, Object>() : jdbcTemplate.queryForMap("SELECT
			// p.posid, p.posno, p.posname, p.mac, p.shopid, s.buplace, s.buarea, s.shopno,
			// s.shopname, s.costcenter, s.descabvrth FROM MASPOS p INNER JOIN MASSHOP s ON
			// s.shopid = p.shopid WHERE p.posid = ?", posIds.get(0));
			// Map<String, Object> sessionInfo = sessId == null ? null :
			// jdbcTemplate.queryForMap("SELECT sessionid, username, firsttime, lastupdated
			// FROM CORSESSION WHERE sessionid = ?", sessId);
			Calendar calendar = new GregorianCalendar(ENGLISH);
			calendar.set(Calendar.HOUR_OF_DAY, 0);
			calendar.set(Calendar.MINUTE, 0);
			calendar.set(Calendar.SECOND, 0);
			// if (sessId == null || sessionInfo == null ||
			// (sessionInfo.containsKey("lastupdated") &&
			// calendar.getTime().after((java.util.Date) sessionInfo.get("lastupdated")))) {
			// sessId = jdbcTemplate.queryForObject("SELECT session_seq.nextval FROM dual",
			// Long.class);
			// }

			this.sessionId = sessId;
			this.posId = 0L;
			// Long.valueOf(userInfo.get("posid")+"");
			this.branchId = 0L;
			// Long.valueOf(userInfo.get("shopid")+"");
			...
			this.roleId = (userInfo.get("roleid") == null) ? -1L : Long.valueOf(userInfo.get("roleid") + "");
			...
			this.posName = "";
			this.macAddress = "";
			this.isPositive = !userInfo.containsKey("ispositive") ? -1
					: Integer.valueOf(userInfo.get("ispositive") + "");
			...
			if ("Suppervisor".equalsIgnoreCase(roleName)) {
				...
				// // 22082017 PICHT
				// +"inner join masofficer o on op.OFFICERID = o.OFFICERID AND O.PRINCIPALID NOT
				// IN ('8') "
				// // END 22082017 PICHT
				// // +"inner join masofficer o on op.OFFICERID = o.OFFICERID "
				// +"where p.SHOPID = ?"
				// +" order by o.username desc "
