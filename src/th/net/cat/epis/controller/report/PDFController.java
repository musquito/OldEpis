...
public class PDFController implements ServletContextAware {
    ...
    private void pushReportToOutputStream(HttpServletResponse resp,Map<String, Object> parameters ,String fileName ) throws IOException, JRException{//HttpServletResponse response, List<JasperPrint> jasperPrints) throws IOException, JRException  {   
    	try{
...
//    		JasperPrint print = JasperFillManager.fillReport(jreport, new HashMap<String, String>());
    		// export it!
...
//		exporter.exportReport(); 
	...
	public void printPaymentReceipt(HttpServletRequest request, HttpServletResponse response, PrintReceiptDTO printReceiptDTO, List<Invoice> invoiceList
    	...
    	Long manualId = printReceiptDTO.getManualId();
    	ManualEntity  manualEntity =  manualRepository.findOne(manualId);
    	List<DeductionManualEntity> deductList = deductionManualRepository.findByManualId(manualId);
    	PayInvoiceManualEntity payInvoice = payInvoiceManualRepository.findByManualId(manualId);
    	List<TrsMethodManualEntity> methodList = trsMethodManualRepository.findByManualId(manualId);
    	System.out.println("printPaymentReceipt");
    	String JASPER_JRXML_FILENAME = "EpisPaymentReceipt";//(printReceiptDTO, invoiceList);
//		String receiptLang = "TH";
//		String billingGroup =payInvoice.getCusSegment();
...
//    	ReceiptOffline receiptOffline= new ReceiptOffline();
...
//		Customer cust = printReceiptDTO.getSettlePaymentDTO().getCustomers().get(0);
		obj.setBillingGroup(payInvoice.getCusSegment());//
//		obj.setCustomerSubNo(cust.getCustSubNo());
		obj.setBranchName("สนามบินน้ำ");
		obj.setBranchCode("100");
		obj.setCustomerNo(manualEntity.getAccountNo());
		obj.setAddressLine1(payInvoice.getCusAddress());
		obj.setReceiptType("FULL");
//		obj.setCustomerNo(payInvoice);
		obj.setCustomerName(payInvoice.getCusName());
		obj.setTaxId(payInvoice.getTaxNo());
		obj.setBranchPayment(BA);
		obj.setPaymentType(methodList.get(0).getName());
		obj.setDocumentNo(manualEntity.getReceiptNoManual());
		obj.setDocumentDate(new SimpleDateFormat(DateUtil.STANDARD_DATE_TIME_PATTERN_NO_SECONDS, ENGLISH).format(manualEntity.getPaidDate()));//rct.getPayment().getDate().toString()manualEntity.getPaidDate());
		obj.setInvoiceNo(manualEntity.getInvoiceNo());
		obj.setPaymentCycle(formatDateDoc(payInvoice.getPeriod()));
