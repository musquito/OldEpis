...
/*
	...
	public Session getSession() {
		Long sessionId = 4101L;//EpContextHolder.getContext().getSessionId();
		Session session = null;//sessionRepo.findOne(sessionId);
		if (session == null) {
			session = new Session();
			session.setId(sessionId);
			session.setUserName("Epis0");//EpContextHolder.getContext().getUserName());
	...
	public boolean selectCheckAuthenSuperwiser(String user,String pass) {
		...
		if(officer!=null
				&& new Md5PasswordEncoder().isPasswordValid(result.get(0).get("PASSWORD")+"", pass, null)
